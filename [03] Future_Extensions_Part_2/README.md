...
This folder contains two subfolders and several files at the root level. The `data` folder includes all datasets used or generated by the scripts, as well as `cdc_query_criteria.md`, which documents the exact CDC WONDER query filters used to obtain the mortality data. The `scripts` folder contains two R scripts. The file `Journal_of_Health_Economics.pdf` is the reference paper by Borgschulte & Vogler (2020)
